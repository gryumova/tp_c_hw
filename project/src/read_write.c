...
int write_file(size_t k, const char* filename, const char action[]) {
  ...
  Task_t *info = (Task_t *)calloc(1, sizeof(Task_t));
  if (info == NULL) {
    puts("Memory error");
    if (fclose(database)) {
  ...
  while (i < k) {
    ...
    if (strlen(info->description) && info->date.day && info->date.mn &&
        info->date.year) {
      fwrite(info, sizeof(Task_t), 1, database);
    } else {
...
TaskList_t *read_file(const char* filename) {
  ...
  Task_t *get_data = (Task_t *)malloc(sizeof(Task_t));
  if (get_data == NULL) {
    puts("Memory error");
    fclose(database);
    free(get_data);
  ...
  if (fread(get_data, sizeof(Task_t), 1, database) == 0) {
    puts("FILE IS EMPTY");
    fclose(database);
    free(get_data);
    return NULL;
  ...
  while (fread(get_data, sizeof(Task_t), 1, database) != 0) {
