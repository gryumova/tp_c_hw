...
int write_file(const size_t k, const char filename[], const char action[]) {
    ...
    size_t i = 0;
    while (i < k) {
        if (fscanf(stdin, "%10lu%2d%30s%2d.%2d.%4d", &info->id_task, &info->priority, info->description, &info->date.day, &info->date.mn, &info->date.year) < 0) {
        ...
        if (strlen(info->description) && info->date.day && info->date.mn && info->date.year) {
