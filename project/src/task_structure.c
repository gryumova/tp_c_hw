...
TaskList_t *add_elem(Task_t *new_task, TaskList_t *head) {
  ...
  if (current->data.priority >= new_task->priority) {
    if (current->data.priority == new_task->priority &&
        !date_comparison(current->data.date, new_task->date)) {
...
TaskList_t *initialise(Task_t *elem) {
  ...
  snprintf(current->data.description, sizeof(elem->description), "%s",
           elem->description);
...
TaskList_t *insert_task(TaskList_t *task, Task_t *elem) {
  ...
  if (snprintf(new_elem->data.description, sizeof(elem->description), "%s",
               elem->description) < 0) {
    return NULL;
  }
